...
<div style="text-align: justify">The goal was to find some drugs that are able to reverse the cancer induced gene expression. The more negative the RGES the higher the drugs ability to reverse the cancer induced gene expression, therefore negative RGES values are searched. Unexpectedly, no negative value was computed. This could be matter to the fact, that all drugs are killing the cancer cells before they are even able to reverse any gene expression. So a positive RGES does not mean, the drug is not effective but that it might be able to somehow stop the cancer growth with other mechanisms rather than interfering with the gene expression. Another reason might be the computation of the RGES values. Bin Chen et al. did not explain, how they computed the disease/drug/gene signatures, so we decided to use the log2-fold-change values. If this was not correct to do, the results are not right either and other signatures would have lead to other results.
</div> 
