...
```{r, echo= FALSE, include=TRUE}
plot(rges.ic50$RGES, rges.ic50$IC50.value, pch=20, col="blue", xlab = "RGES", ylab = "Drug Senistivity", main = "Scatterplot RGES - Drug Sensitivity")
```
...
The predicted values are shown in a vector, there will be presented only the first ten:
```{r results = 'asis', echo=FALSE, include=TRUE} 
knitr::kable(pred [1:10], caption = "Predicted drug sensitivity values") 
```
...
In ordert to prove the correlation between residuals and variables, the correlation coefficient is computed.
...
```
```{r results = 'asis', echo=FALSE, include=TRUE} 
knitr::kable(predict.multiple, caption = "Predicted values") 
```
...
```
```{r results = 'asis', echo=FALSE, include=TRUE} 
knitr::kable(predict.biomarkers, caption = "Predicted values") 
```
