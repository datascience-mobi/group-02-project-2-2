...
---
The goal of the following broad analysis is to gain an overview of the given datasets containing the influence of different drugs to the cellular response. This will be the basis for all next investigations, containing one specific drug - cisplatin. 
...
In order to clean up our data, a loop for checking the amount of NAs in every matrix was created. Rows/Columns containing only NAs were deleted, because they do not contain any information. This only concerned the "mutations" matrix, and two columns were deleted.
Furthermore the data has been scaled for the following investigations.
```{r, include = FALSE}
...
In order to analyse the gene expression change from untreated to treated, log2 fold change values were computed. Because the data given was already log2-transformed, computing the difference between the treated(scaled) and untreated(scaled) data was enough. The distribution of the fold change values was checked through a density plot. The values are normally distributed and located near to 0.
In the future, it would have also been interesting to explore the density of the fold change values by each cell line and not by all cell lines together.
```{r}
...
For a dimensionallity reduction, a principal component analysis was performed. Component 4 was chosen as the last significant component, regarding to the elbow plot. 
```{r}
...
The PCA was interpreted by plotting the first four components in different plots, and coloring different groups of data.
...
Firstly, each drug was colored in a different color. PC1 and PC2 don't seem to show any separations, but PC3 and PC4 seem to seperate the diffenrent drugs, especially Vorinostat an Bortezomib.
...
This could be because vorinostat is the only HDAC inhibitor. It is a cytostatic drug used in T-cell lymphomas and therefor it is very specific. Bortezomib is the only Proteasome inhibitor and causes the loss of function of the proteasom. The loss of function of the proteasome results in the inhibition of different proteins, what leads to apoptosis and downregulation of the cell cycle. 
...
Lastly, the 4 tyrosine kinase inhibitor (TKI) drugs were colored. There was no seperation visible in PC1 and PC2. By plotting PC3 and PC4, a batch of only TKI became visible. Although there are also many different drugs located in the same area, the concentration of the TKI may result from the specific and targeted mechanism.
